...
shell: image ## Run a shell in a new container created from image
	docker run -a stdin -a stdout -i -t --rm --name "nginx_proxy" --entrypoint=/bin/sh $(APP_NAME)

test: image ## Run a test webserver with default configuration on port 80 and 443
	docker run --rm --name "nginx_proxy"  -v $(PWD)/test/config/sites-enabled:$(NGINX_OPTS)/sites-enabled -v $(PWD)/test/certs:/etc/nginx/ssl -v $(PWD)/test/webcontent:$(WEB_DOCUMENT_ROOT) -p $(TEST_HTTP_PORT):80 -p $(TEST_HTTPS_PORT):443 $(APP_NAME)
